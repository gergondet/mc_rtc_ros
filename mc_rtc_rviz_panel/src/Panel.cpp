...
void savePanelConfiguration(const mc_rtc::Configuration & config)
{
  ...
  {
    mc_rtc::log::error("Cannot save configuration to {0}, {0} is not a directory", config_directory.string());
    return;
  }
  auto config_path = getConfigPath();
  if(bfs::exists(config_path) && !bfs::is_regular(config_path))
  {
    mc_rtc::log::error("Cannot save configuration to {0}, {0} is not a regular file", config_path.string());
    return;
  }
